    ...
    for (i = 0, c = 0; i < ny; i++) {
        for (j = 0; j < nx; j++, c++) {
            if (i == 0 && j == 0) { //CANTO SUPERIOR ESQUERDO VOLUME 1
                if (dbo)printf("C1");
                if (dbl)fprintf(fl, "C1");
                Ae[i][j] = (1 / 2 - ax) * ro * u *dy + bx * ga*dy / dx;
                Aw[i][j] = 0;
                An[i][j] = 0;
                As[i][j] = -ro * v *dx* (1 / 2 - ay) + (by * ga*dx) / dy;
                Ap[i][j] = (2 * bx * ga*dy) / dx + (2 * by * ga*dx) / dy;
                B[c] = q;
            } else if (i == 0 && j == nx - 1) {//CANTO SUP DIR VOLUME NX
                if (dbo)printf("C2");
                if (dbl)fprintf(fl, "C2");
                Ae[i][j] = 0;
                Aw[i][j] = -ro * u * dy * (1 / 2 - ax) + bx * ga * dy / dx;
                An[i][j] = 0;
                As[i][j] = -ro * v * dx *(1 / 2 - ay) + by * ga *dx / dy;
                Ap[i][j] = +2 * bx * ga * dy / dx + 2 * by * ga * dx / dy;
                B[c] = q;
                ...
                if (dbo)printf("C3");
                if (dbl)fprintf(fl, "C3");
                Ae[i][j] = (1 / 2 - ax) * ro * u * dy + bx * ga * dy / dx;
                Aw[i][j] = 0;
                An[i][j] = (1 / 2 - ay) * ro * v * dx + by * ga * dx / dy;
                As[i][j] = 0;
                Ap[i][j] = 2 * bx * ga * dy / dx + 2 * by * ga * dx / dy;
                B[c] = q + ro * v * sin(PI * (j * dx + dx / 2) / a) + 2 / dy * sin(PI * (j * dx + dx / 2) / a);
            } else if (i == ny - 1 && j == nx - 1) { //CANTO INFERIOR DIREITO
                if (dbo)printf("C4");
                if (dbl)fprintf(fl, "C4");
                Ae[i][j] = 0;
                Aw[i][j] = -ro * u * dy * (1 / 2 - ax) + bx * ga * dy / dx;
                An[i][j] = (1 / 2 - ay) * ro * v * dx + by * ga * dx / dy;
                As[i][j] = 0;
                ...
                if (dbo)printf("LE");
                if (dbl)fprintf(fl, "LE");
                Ae[i][j] = (1 / 2 - ax) * ro * u * dy + bx * ga * dy / dx;
                Aw[i][j] = 0;
                An[i][j] = (1 / 2 - ay) * ro * v * dx + by * ga * dx / dy;
                As[i][j] = -ro * v * dx * (1 / 2 - ay) + by * ga * dx / dy;
                Ap[i][j] = 2 * bx * ga * dy / dx;
                B[c] = q;
            } else if (j == nx - 1 && i > 0 && i < ny - 1) { //LATERAL DIREITA
                if (dbo)printf("LD");
                if (dbl)fprintf(fl, "LD");
                Ae[i][j] = 0;
                Aw[i][j] = -ro * u * dy * (1 / 2 - ax) + bx * ga * dy / dx;
                An[i][j] = (1 / 2 - ay) * ro * v * dx + by * ga * dx / dy;
                As[i][j] = -ro * v * dx * (1 / 2 - ay) + by * ga * dx / dy;
                ...
                if (dbo)printf("TP");
                if (dbl)fprintf(fl, "TP");
                Ae[i][j] = (1 / 2 - ax) * ro * u * dy + bx * ga * dy / dx;
                Aw[i][j] = -ro * u * dy * (1 / 2 - ax) + bx * ga * dy / dx;
                An[i][j] = 0;
                As[i][j] = -ro * v * dx * (1 / 2 - ay) + by * ga * dx / dy;
                Ap[i][j] = 2 * by * ga * dx / dy;
                B[c] = q;
            } else if (i == ny - 1 && j > 0 && j < nx - 1) { //INFERIOR
                if (dbo)printf("IN");
                if (dbl)fprintf(fl, "IN");
                Ae[i][j] = (1 / 2 - ax) * ro * u * dy + bx * ga * dy / dx;
                Aw[i][j] = -ro * u * dy * (1 / 2 - ax) + bx * ga * dy  / dx;
                An[i][j] = (1 / 2 - ay) * ro * v * dx + by * ga * dx / dy;
                As[i][j] = 0;
